...
```{r}
set.seed(123)
system.time(out_dd <- mvglm_obs_dd(x, n_permutations=100))
out_dd #65s for 100, p = 0.099

system.time(out_dd2 <- mvglm_obs(x, 
                          add_var = "days.fished",
                          block = "BLOCK",
                          n_permutations = 100,
                          xi.vec = seq(1.2, 1.8, length.out=5)
                          )) 
out_dd2$results #83s for 100, p = 0.168 (halibut different)
```
