...
perf_plot_dat <- 
  ...
  summarize(psig = mean(p<=0.05), 
            num = n(),
            se = sqrt((psig * (1 - psig)) / num), .groups="drop") %>%
  inner_join(params_to_test, by=c("param_mod" = "param")) %>%
  mutate(Test = factor(Test, levels = c("perma_p", "mvglm_p"),
                       labels = c("PERMANOVA", "MvGLM")),
         #fix panel titles
...
pd <- position_dodge(width = 0.4) # Now 0.4 means "40% of the gap between points"
...
perf_plot <- ggplot(perf_plot_dat, 
                    aes(x = factor(param_value), # Treat X as labels, not numbers
                        ...
                        linetype = Test,
                        fill = Test, 
                        group = Test)) +         # Group keeps the lines connected
