...
##data generation
**make trip data x iterations external to this script and pass in those
**with target bp
...
##all stats together
```{r}
rr <- ObserverEffectStats(test_trips, metrics=c("sp_1","sp_2","biomass_total"), nboot_triplet=100, nperm_mvglm = 10)
rr 
```
...
```{r}
test_trips
Y <- test_trips %>% dplyr::select(sp_1, sp_2)
adon <- vegan::adonis2(Y ~ factor(obs), data = test_trips, permutations = 99, method="euclidean")
adon
```
