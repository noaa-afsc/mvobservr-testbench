...
save(trip_sets, trip_sets_adj, res_g, res_p, res_t, res_tt, file = allbutmv_name)
# Upload to the Google Shared Drive
gdrive_upload(allbutmv_name, output_dribble)
```
...
alltests_name <- paste0("output_data/alltests_2spp_oe", max(abs(bias))*100, "_cov", trip_coverage*100, "_set", set_number, ".Rdata")
save(res_comb, file = paste0(alltests_name, set_number, ".Rdata"))
gdrive_upload(alltests_name, output_dribble)
```

###combine data from multiple sets
```{r}

alltests_name.set <- paste0("output_data/alltests_2spp_oe", max(abs(bias))*100, "_cov", trip_coverage*100, "_set")
load(gdrive_download(paste0(alltests_name.set, 1, ".Rdata"), output_dribble))
r1 <- res_comb
load(gdrive_download(paste0(alltests_name.set, 2, ".Rdata"), output_dribble))
r2 <- res_comb
