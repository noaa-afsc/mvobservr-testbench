...
```{r falsepos setup, eval=FALSE}
#replace standard values for test of no bias with 50% BP
target_bp_levels <- 0.5
bias <- c(0, 0)
```
...
res_m <- imap(1:length(trip_sets_adj), ~{
  ...
  trip_sets_adj[[.x]] %>%
      ...
      mvglm_obs(block = NULL, add_var = NULL, n_permutations = 100, nCores = TRUE) %>%
      pluck("results") %>%
